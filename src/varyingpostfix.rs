...
// const totally_safe_name_trust_me_im_a_cat: &'static str = {unsafe{&std::str::from_utf8_unchecked(&const {
//     let mut s: [u8; name.as_bytes().len()+postfix.as_bytes().len()+1] = [0u8; name.as_bytes().len()+postfix.as_bytes().len()+1];
...
const totally_safe_name_trust_me_im_a_cat: &'static str = gen_concat!(name, postfix); 
